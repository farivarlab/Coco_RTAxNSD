...
def compute_and_save_roi_rdms(base_path, roi_path, roi_labels_txt, selected_rois, n_sessions=40, download_npy=False):
    ...
    selected_indices = np.arange(30000) # no random sampling
...
rsa_matrix = np.zeros((len(selected_rois), len(selected_rois)))

for i in range(len(selected_rois)):
    roi_i = selected_rois[i]
    rdm_i = np.load(join(base_path, f"{roi_i}_RDM.npy"))  # Load from .npy file
    for j in range(i, len(selected_rois)):
        roi_j = selected_rois[j]
        rdm_j = np.load(join(base_path, f"{roi_j}_RDM.npy"))  # Load from .npy file
        rsa_score = compare_rdms(rdm_i, rdm_j)
...
plt.tight_layout()
plt.savefig(join(base_path, "7roi_RSA.png"))
plt.show()            
